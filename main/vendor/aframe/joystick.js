...
// const style = "position:fixed;display:block;width:150px;height:150px;left:50px;bottom:50px;opacity:0.5;z-index:20;"
...
p.setAttribute("style","text-align: center; color: white; margin-top:40px;font-size:16px; font-family: Trebuchet MS, Helvetica, sans-serif; opacity:1;");
p.innerHTML=""
d.appendChild(p)
}
...
function createJoystick() {
  ...
  function updatePosition(data) {
    ...
    x = cam.getAttribute("position")["x"] + f/100*(x_vec ) ;
    y = cam.getAttribute("position")["y"]
    z = cam.getAttribute("position")["z"] - f/100*(y_vec ) ;
  ...
  AFRAME.registerComponent('joystick', {
    ...
    init: function() { 
      ...
      this.nav = this.sceneEl.systems.nav;
      if (this.nav) {
    ...
    //   var el = this.el;
    //   var data = this.data;
    //   var nav = this.sceneEl.systems.nav;
    ...
    updatePosition: function(data) {
      ...
      start.copy(this.cam.object3D.position);
      f = data.force;
      ang = data.angle.radian
      
      
  
      x_vec = Math.cos(ang + 3.14/180*this.cam.getAttribute('rotation')['y']) ;
      y_vec = Math.sin(ang + 3.14/180*this.cam.getAttribute('rotation')['y']) ;
  
      end.x = this.cam.getAttribute("position")["x"] + f/20*(x_vec ) ;
      end.y = this.cam.getAttribute("position")["y"]
      end.z = this.cam.getAttribute("position")["z"] - f/20*(y_vec ) ;
      // cam.setAttribute("position",`${x} ${y} ${z}`)
      
      // const nav = this.sceneEl.systems.nav;
      if (this.data.useNavmesh) { //dang it, navGroup is undefined //touch-control just triggers on id='np'
        console.log("tryna use navmesh");
        ...
        // this.navNode = nav.clampStep(start, end, this.navGroup, this.navNode, clampedEnd);  //constrain to navmesh!
        // this.cam.object3D.position.copy(clampedEnd);
      } else {
        this.cam.setAttribute("position",`${end.x} ${end.y} ${end.z}`);
      }
    }
  });
